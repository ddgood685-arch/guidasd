...
local function CreateTabIcon(parent, iconType, sbWidth, iconColor)
    local sz = RS(20, 18)
    local xOff = RS(18, 12)
    local color = iconColor or Theme.Accent
    ...
    local function Line(w, h, px, py, rot)
        ...
        l.ZIndex = 10
        l.Parent = c
        Corner(l, UDim.new(1, 0))
        return l
    end

    local function Ring(diameter, px, py, thickness)
        local outer = Instance.new("Frame")
        outer.Size = UDim2.new(0, diameter, 0, diameter)
        outer.AnchorPoint = Vector2.new(0.5, 0.5)
        outer.Position = UDim2.new(0, px, 0, py)
    ...
    local function Dot(diameter, px, py)
        local d = Instance.new("Frame")
        d.Size = UDim2.new(0, diameter, 0, diameter)
    ...
    local mid = math.floor(sz / 2)

    -- ══════════════════════════════════════════
    -- BOLT  ⚡  (zigzag lightning line)
    -- ══════════════════════════════════════════
    if iconType == "bolt" then
        Line(2, 6, mid - 2, 3, -15)        -- top stroke going left
        Line(2, 5, mid, mid, 15)            -- middle stroke going right
        Line(2, 6, mid - 2, sz - 3, -15)   -- bottom stroke going left
        Dot(3, mid + 2, mid)                -- spark dot at bend

    -- ══════════════════════════════════════════
    -- TARGET  ◎  (crosshair with center dot)
    -- ══════════════════════════════════════════
    elseif iconType == "target" then
        Ring(sz - 4, mid, mid, 1.5)         -- outer ring
        Dot(4, mid, mid)                    -- center dot
        Line(6, 1.5, mid, 2)               -- top line
        Line(6, 1.5, mid, sz - 2)          -- bottom line
        Line(1.5, 6, 2, mid)               -- left line
        Line(1.5, 6, sz - 2, mid)          -- right line
    ...
    elseif iconType == "shield" then
        ...
        Line(2, sz - 8, 3, mid - 1)
        -- Right side
        Line(2, sz - 8, sz - 3, mid - 1)
        -- Bottom left diagonal
        Line(2, 6, 5, sz - 5, 25)
        -- Bottom right diagonal
        Line(2, 6, sz - 5, sz - 5, -25)
        -- Bottom point
        Dot(3, mid, sz - 2)
        -- Inner check mark
        Line(2, 4, mid - 2, mid + 1, -30)
        Line(2, 6, mid + 2, mid - 1, 25)

    -- ══════════════════════════════════════════
    -- STAR  ★  (5-point star from lines)
    -- ══════════════════════════════════════════
    elseif iconType == "star" then
        Dot(4, mid, 3)                     -- top point
        Line(2, 5, mid - 4, mid + 1, 20)  -- top-left arm
        Line(2, 5, mid + 4, mid + 1, -20) -- top-right arm
        Line(7, 1.5, mid, mid + 2)        -- middle crossbar
        Line(2, 5, mid - 3, sz - 3, -15)  -- bottom-left leg
        Line(2, 5, mid + 3, sz - 3, 15)   -- bottom-right leg

    -- ══════════════════════════════════════════
    -- CIRCLE  ●  (ring with dot)
    -- ══════════════════════════════════════════
    elseif iconType == "circle" then
        Ring(sz - 4, mid, mid, 1.5)
        Dot(5, mid, mid)
    ...
    elseif iconType == "square" then
        Line(sz - 4, 2, mid, 3)            -- top
        Line(sz - 4, 2, mid, sz - 3)       -- bottom
        Line(2, sz - 4, 3, mid)            -- left
        Line(2, sz - 4, sz - 3, mid)       -- right
        Dot(3, mid, mid)                   -- center dot

    -- ══════════════════════════════════════════
    -- DIAMOND  ◆  (rotated square outline)
    -- ══════════════════════════════════════════
    elseif iconType == "diamond" then
        Line(2, 8, mid, mid - 3, 45)       -- top-right edge
        Line(2, 8, mid, mid + 3, 45)       -- bottom-right edge
        Line(2, 8, mid, mid - 3, -45)      -- top-left edge
        Line(2, 8, mid, mid + 3, -45)      -- bottom-left edge
        Dot(3, mid, mid)

    -- ══════════════════════════════════════════
    -- BARS  ≡  (3 horizontal lines - menu)
    -- ══════════════════════════════════════════
    elseif iconType == "bars" or iconType == "list" then
        Line(sz - 6, 2, mid, 5)
        Line(sz - 8, 2, mid, mid)
        Line(sz - 10, 2, mid, sz - 5)
        -- Left dots
        Dot(2.5, 4, 5)
        Dot(2.5, 4, mid)
        Dot(2.5, 4, sz - 5)
    ...
    elseif iconType == "triangle" then
        Line(2, sz - 6, 5, mid)            -- left vertical
        Line(2, 8, 8, 5, -30)             -- top diagonal
        Line(2, 8, 8, sz - 5, 30)         -- bottom diagonal
        Dot(3, sz - 4, mid)               -- right tip

    -- ══════════════════════════════════════════
    -- DOT-GRID  ⊞  (4 dots in grid)
    -- ══════════════════════════════════════════
    elseif iconType == "dot-grid" then
        local gap = math.floor(sz / 3)
        Dot(3.5, gap, gap)
        Dot(3.5, sz - gap, gap)
        Dot(3.5, gap, sz - gap)
        Dot(3.5, sz - gap, sz - gap)
        -- Connecting lines
        Line(sz - gap * 2, 1, mid, gap)          -- top horizontal
        Line(sz - gap * 2, 1, mid, sz - gap)     -- bottom horizontal
        Line(1, sz - gap * 2, gap, mid)           -- left vertical
        Line(1, sz - gap * 2, sz - gap, mid)     -- right vertical

    -- ══════════════════════════════════════════
    -- SETTINGS  ⚙  (gear)
    -- ══════════════════════════════════════════
    elseif iconType == "settings" then
        Ring(sz - 8, mid, mid, 1.5)        -- inner ring
        Dot(3, mid, mid)                   -- center
        -- Gear teeth (8 lines radiating out)
        for a = 0, 315, 45 do
            local rad = math.rad(a)
            local r = (sz / 2) - 1
            local px = mid + math.cos(rad) * r
            local py = mid + math.sin(rad) * r
            Line(2, 4, px, py, a + 90)
        end
    ...
    -- FIRE  ◈  (flame shape)
    -- ══════════════════════════════════════════
    elseif iconType == "fire" then
        -- Outer flame
        Line(2, 7, mid - 2, mid - 2, -10)
        Line(2, 7, mid + 2, mid - 2, 10)
        -- Inner flame
        Line(2, 5, mid, mid + 1, 0)
        -- Tip
        Dot(3, mid, 2)
        -- Base
        Line(8, 2, mid, sz - 3)
    ...
    -- CROWN  ♛  (crown outline)
    -- ══════════════════════════════════════════
    elseif iconType == "crown" then
        -- Base
        Line(sz - 4, 2, mid, sz - 3)
        -- Left pillar
        Line(2, sz - 8, 4, mid - 1)
        -- Right pillar
        Line(2, sz - 8, sz - 4, mid - 1)
        -- Center spike
        Line(2, 5, mid, 3)
        -- Left spike
        Dot(3, 4, 4)
    ...
    elseif iconType == "skull" then
        Ring(sz - 4, mid, mid - 2, 1.5)   -- head outline
        -- Eyes
        Dot(3, mid - 4, mid - 2)
        Dot(3, mid + 4, mid - 2)
        -- Nose
        Line(1.5, 3, mid, mid + 2)
        -- Jaw lines
        Line(3, 1.5, mid - 3, sz - 3)
        Line(3, 1.5, mid, sz - 3)
        Line(3, 1.5, mid + 3, sz - 3)

    -- ══════════════════════════════════════════
    -- COIN  $  (coin circle with line)
    -- ══════════════════════════════════════════
    elseif iconType == "coin" then
        Ring(sz - 4, mid, mid, 1.5)
        Line(1.5, sz - 8, mid, mid)        -- vertical center line
        -- S curves
        Line(5, 1.5, mid, mid - 3)
        Line(5, 1.5, mid, mid + 3)

    -- ══════════════════════════════════════════
    -- TRASH  ✕  (trash can outline)
    -- ══════════════════════════════════════════
    elseif iconType == "trash" then
        -- Lid
        Line(sz - 4, 2, mid, 4)
        Line(4, 1.5, mid, 2)              -- handle
        -- Body
        Line(2, sz - 8, 5, mid + 2)       -- left wall
        Line(2, sz - 8, sz - 5, mid + 2)  -- right wall
        Line(sz - 8, 2, mid, sz - 2)      -- bottom
        -- Inner lines
        Line(1.5, sz - 10, mid - 3, mid + 2)
        Line(1.5, sz - 10, mid + 3, mid + 2)

    -- ══════════════════════════════════════════
    -- ARROW-UP  ▲  (up arrow)
    -- ══════════════════════════════════════════
    elseif iconType == "arrow-up" then
        Line(1.5, sz - 6, mid, mid)        -- shaft
        Dot(3, mid, 3)                     -- tip
        Line(2, 6, mid - 4, 6, 35)        -- left arrowhead
        Line(2, 6, mid + 4, 6, -35)       -- right arrowhead

    -- ══════════════════════════════════════════
    -- REFRESH  ⟳  (circular arrow)
    -- ══════════════════════════════════════════
    elseif iconType == "refresh" then
        -- Partial ring (3/4)
        Ring(sz - 4, mid, mid, 1.5)
        -- Gap cover (hides part of ring to make it look partial)
        local gap2 = Instance.new("Frame")
        gap2.Size = UDim2.new(0, 6, 0, 6)
        gap2.AnchorPoint = Vector2.new(0.5, 0.5)
        gap2.Position = UDim2.new(0, mid + 4, 0, 3)
        gap2.BackgroundColor3 = parent.BackgroundColor3
        gap2.BorderSizePixel = 0
        gap2.ZIndex = 11
        gap2.Parent = c
        -- Arrow tip
        Dot(3, mid + 5, 5)
    ...
    -- WEBHOOK  🔗  (chain link)
    -- ══════════════════════════════════════════
    elseif iconType == "webhook" then
        -- Two interlocking rings
        Ring(sz - 8, mid - 3, mid - 2, 1.5)
        Ring(sz - 8, mid + 3, mid + 2, 1.5)
        -- Connection line
        Line(6, 1.5, mid, mid, 45)
